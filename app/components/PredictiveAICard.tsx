...
export default function PredictiveAICard({
  ...
  return (
    <div className="bg-[#0f0f0f] rounded-2xl border border-slate-800 p-6 shadow-xl relative overflow-hidden">
      ...
      <div className="flex flex-wrap justify-between items-start gap-4 mb-6 border-b border-slate-800/80 pb-5">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-white text-base font-bold tracking-wide flex items-center gap-2">
              ...
              <span className="text-[10px] bg-sky-950 text-sky-400 border border-sky-800/60 px-2 py-0.5 rounded font-mono font-semibold">
                v2.4
              </span>
            </h3>
          </div>
          <p className="text-slate-400 text-xs">
            Ramalan siri masa paras air berasaskan {historicalCount} rekod lampau Supabase
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            {showForecastOnChart ? 'Unjuran Graf Aktif' : 'Unjuran Ditutup'}
          </button>
        </div>
      </div>

      {/* 2. Kawalan Model & Ambang Bahaya */}
      <div className="bg-[#141414] rounded-xl border border-slate-800/80 p-3.5 mb-6 flex flex-wrap justify-between items-center gap-4 text-xs">
        {/* Pilihan Model */}
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <span className="text-slate-400 font-medium flex items-center gap-1.5">
            <AlertTriangle size={13} className="text-red-400" /> Paras Bahaya:
          </span>
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              className="w-16 bg-[#0a0a0a] border border-slate-800 text-white font-mono text-xs px-2 py-1 rounded-lg text-center focus:outline-none focus:border-indigo-500"
          ...
          <div className="flex bg-[#0a0a0a] p-1 rounded-lg border border-slate-800">
            {[15, 30, 60].map((mins) => (
              <button
                ...
                className={`px-2.5 py-0.5 rounded-md text-[11px] font-bold font-mono transition-all ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className={`p-4 rounded-xl border relative overflow-hidden transition-all ${
          ...
          <div className="mt-1">
            {isCurrentlyCritical ? (
              <div>
                <p className="text-2xl font-black text-red-400 tracking-tight animate-pulse flex items-center gap-2">
                  <Flame size={22} /> PARAS BAHAYA!
                </p>
              ...
              <div>
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-sm font-bold text-slate-300 uppercase">Minit Lagi</span>
                </div>
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xl font-bold text-emerald-400">TIADA ANCAMAN</span>
                </div>
                <p className="text-xs text-slate-400 mt-1">
                  {trend === 'FALLING' 
          ...
          <div className="flex items-center gap-1.5 mt-2 text-xs">
            {trend === 'RISING' ? (
              <span className="text-amber-400 flex items-center gap-1 font-semibold">
                <TrendingUp size={14} /> Tren Menaik Pantas
              </span>
            ) : trend === 'FALLING' ? (
              <span className="text-emerald-400 flex items-center gap-1 font-semibold">
                <TrendingDown size={14} /> Tren Sedang Surut
              </span>
            ) : (
              <span className="text-slate-400 flex items-center gap-1 font-semibold">
                <Minus size={14} /> Paras Air Mendatar/Stabil
              </span>
            )}
        ...
        <div className="bg-[#141414] p-4 rounded-xl border border-slate-800">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] font-mono text-indigo-400 font-bold bg-indigo-950/60 px-1.5 py-0.5 rounded border border-indigo-900/50">
              {rSquared >= 80 ? 'Akurasi Tinggi' : rSquared >= 50 ? 'Sederhana' : 'Dinamik'}
            </span>
          </div>
          <div className="flex items-baseline gap-2">
      ...
      <div className="bg-[#141414] rounded-xl border border-slate-800/80 p-4">
          ...
          {[
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10px] font-black uppercase mt-1 inline-block px-1.5 py-0.5 rounded ${
                  isDanger 
